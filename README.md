...
rag_project/
├── src/rag_app/
│   ├── cli.py           # команды index, search, ask, chat и serve
│   ├── api.py           # постоянный HTTP API для запросов и индексации
...
Сервис `app` по умолчанию запускается как постоянный HTTP API. Один `RagService`
создаётся при старте и переиспользует загруженные embedding-модель, reranker и BM25
между запросами:

```bash
docker compose build app
docker compose up -d qdrant vllm app
...
curl -X POST http://localhost:8080/query \
  ...
  -d '{"question":"Когда заключён договор?","session_id":"user-1"}'
```

`session_id` отделяет историю разных диалогов. Если поле не передано, используется
сессия `default`. Запросы разных сессий выполняются одновременно, а вопросы одной
сессии — последовательно, чтобы ответы не поменяли порядок истории. По умолчанию API
допускает два одновременных RAG-запроса; лимит задаётся через
`RAG_API_MAX_CONCURRENT_QUERIES`. Индексация получает эксклюзивный доступ: она ждёт
активные вопросы и до завершения не запускает новые.

Проиндексировать папку внутри подключённого каталога `/data`:

```bash
curl -X POST http://localhost:8080/ingest \
  -H "Content-Type: application/json" \
  -d '{"path":"npatest","reset":true}'
```

API не разрешает `/ingest` читать пути за пределами `/data`. Состояние загруженных
компонентов доступно через `GET /stats`. Порт на хосте задаётся переменной
`RAG_API_PORT`, а число реплик истории — `RAG_API_HISTORY_TURNS`. FastAPI публикует
интерактивную OpenAPI-документацию по адресу `http://localhost:8080/docs`.
API не выполняет аутентификацию, поэтому его следует открывать только во внутренней
сети либо за reverse proxy с TLS и проверкой доступа.

Без Compose тот же сервер запускается командой:

```bash
rag serve --host 0.0.0.0 --port 8080 --data-root data \
...
По умолчанию vLLM с `RAG_VLLM_TENSOR_PARALLEL_SIZE=1` использует GPU 0, а сервис
`app` получает GPU 1 через `RAG_RETRIEVAL_GPU_DEVICE_ID`. Поэтому embedding запроса
и reranker работают на отдельной карте и не конкурируют с генеративной моделью.
Если номера карт отличаются, задайте нужный номер в `.env`.
...
В режиме `--stream` после ответа отдельно выводятся время поиска контекста, ожидание
первого фрагмента от vLLM, время генерации и полное время запроса. `/clear` удаляет
только историю разговора; загруженные embedding-модель,
...
проверка не прошла. Подробности записываются в JSON-отчёт. Рядом автоматически
создаётся `results-failed.json`: в нём остаются только полные записи с
`passed: false`.
...
В этом примере рядом также создаётся `ragas-results-failed.json` с непройденными
ответами, их контекстами, метриками и ошибками. Файл перезаписывается при каждом
запуске; если все примеры прошли, список `results` будет пустым.

...
названия колонок в каждом чанке. Это позволяет поиску сопоставлять запросы вроде
«максимальная выручка» со строками, содержащими конкретные даты и значения.

...
TXT-файлы содержат полный извлеченный текст, а не короткие выдержки. Это также
позволяет текущему `DocumentProcessor` индексировать бывшие RTF-документы без
добавления отдельного RTF-загрузчика.
